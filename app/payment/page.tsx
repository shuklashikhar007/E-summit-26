        ...
        <div className="flex justify-center items-center mb-4">
          <label className="flex items-center cursor-pointer">
            ...
            <div className="relative">
              <input type="checkbox" className="sr-only" checked={type === 'startup_expo'} onChange={handleToggle} />
              <div className="block bg-gray-700 w-14 h-8 rounded-full shadow-inner"></div>
              <div className={`dot absolute top-1 w-6 h-6 rounded-full transition-transform ${type === 'esummit' ? 'transform translate-x-0 bg-white' : 'transform translate-x-6 bg-blue-500'} shadow`}></div>
            </div>
            <span className={`ml-3 ${type === 'startup_expo' ? 'text-blue-500' : 'text-white'}`}>Startup Expo</span>
          </label>
        </div>
        {type === 'esummit' ? (
          <iframe
